...
- **R13 watch item — validation/builder rule duplication**: `validation.py` deliberately mirrors builder structural rules (error-edge rules exist in both). Not force-shared; R4's per-handler `validate()` registration is the natural convergence point. Guard test **added 2026-09-01** (`backend/tests/test_templates.py`: every bundled workflow template in `backend/app/templates/` loads through `load_workflow` and passes `validate_workflow`; registry samples are covered by `test_registry_samples.py`).
