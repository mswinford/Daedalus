...
- **`latest`:** resolves to the newest **PUBLISHED** version of a name (steering away from DEPRECATED). Versions are immutable; an imported artifact is a *snapshot* — it does not track its source. Upgrading = explicitly re-fetching a newer version, which the editor now automates: update detection against stamped origins + in-place upgrade with a per-field drift diff (R2, shipped).
...
- **Live refs:** runtime-resolved references / opt-in `latest` tracking for existing imports (R2 remainder; the invoke node already resolves at run time).
